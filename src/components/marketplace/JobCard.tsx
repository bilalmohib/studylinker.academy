    ...
    <div className="bg-white rounded-2xl p-4 sm:p-6 shadow-lg hover:shadow-xl transition-all duration-300 border border-gray-100">
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2 mb-2">
            ...
            {status === "open" ? (
              <span className="px-2 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-700 shrink-0">
                Open
              </span>
            ) : (
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-4 text-xs sm:text-sm text-gray-600">
            <span className="flex items-center gap-1">
              <BsBook className="w-3 h-3 sm:w-4 sm:h-4 shrink-0" />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mb-4">
        <div className="flex items-center gap-2 text-xs sm:text-sm text-gray-600">
          <BsClock className="w-3 h-3 sm:w-4 sm:h-4 text-indigo-600 shrink-0" />
          <span>{hoursPerWeek} hours/week</span>
        </div>
        <div className="flex items-center gap-2 text-xs sm:text-sm text-gray-600">
          <BsCurrencyDollar className="w-3 h-3 sm:w-4 sm:h-4 text-indigo-600 shrink-0" />
          <span className="font-semibold text-gray-900">{budget}/month</span>
        </div>
        <div className="flex items-center gap-2 text-xs sm:text-sm text-gray-600">
          <BsPerson className="w-3 h-3 sm:w-4 sm:h-4 text-indigo-600 shrink-0" />
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pt-4 border-t border-gray-200">
        <span className="text-xs sm:text-sm text-gray-500">Posted {postedDate}</span>
        <Button
          asChild
          variant={status === "open" ? "default" : "outline"}
          className={`w-full sm:w-auto text-sm sm:text-base ${status === "open" ? "bg-indigo-600 hover:bg-indigo-700 text-white" : ""}`}
        >
          <Link href={`/jobs/${id}`} className="text-center">
